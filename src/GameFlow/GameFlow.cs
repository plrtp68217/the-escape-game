...
public partial class GameFlow : Node
{
	...
	public override void _Process(double delta)
	{
		...
		if (inGameplay && local.Combat.HasReviveTarget)
		{
			_ui.SetInteractPrompt("Удерживайте [F], чтобы поднять");
			_ui.SetReviveProgress(local.Combat.ReviveProgress);
		}
		else if (inGameplay && local.Combat.IsSelfHealing)
		{
			_ui.SetInteractPrompt("Лечение…");
			_ui.SetReviveProgress(local.Combat.SelfHealProgress);
		}
		else
		{
			_ui.SetInteractPrompt(inGameplay ? local.Interaction.GetInteractPrompt() : string.Empty);
	...
	{
		int filled = Mathf.Clamp(Mathf.RoundToInt(player.Abilities.StaminaRatio * 10f), 0, 10);
	...
	private static string FormatAbility(PlayerController player)
	{
		...
		if (player.Abilities.ScanActive)
		{
			return "Скан активен";
		}

		return player.Abilities.ScanReady ? "Скан [R]: готов" : $"Скан: {player.Abilities.ScanCooldownSeconds}с";
	}
	...
	{
		PlayerController.LocalPlayer?.InventoryController.RequestDropSlot(slotIndex);
	}
